...
В зависимости от levelid информацию о объекте надо искать в разных таблицах (**имя таблицы жирно в скобках**):
...
Таблицы эти называются по аналогии ***_params**. Есть общий справочник параметров **param_types**, где перечисленны все возможные значения ключей.
...
*isactive* - правильная и актуальная информация по объекту на которую и надо ориентироваться при ссылках и в справочниках
...
(5 rows)
```
При формировании новых связей (ссылок) на этот СНТ надо использовать исключительно objectid = 164552691 с флагом isactive (id=52127278), но при этом старые записи ссылающиеся на objectid не теряют своей актуальности, хотя по идее их быть уже не должно.

При работа со справочником мы легко можем выбрать все объекты имеющие какое-то отношение к адресной иерархии. Я использую именно административное деление, так как оно наиболее приближено к "карте и земле".
